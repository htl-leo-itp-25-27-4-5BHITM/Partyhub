...
async function loadAllParties() {
  ...
  try {
    ...
    container.appendChild(
      EmptyState.create("parties", {
        message:
          "Unable to load parties. Please check your connection and try again.",
        actionLabel: "Retry",
...
function applyFilters() {
  filteredParties = allParties.filter((party) => {
    ...
    const matchesSearch =
      ...
      (party.description &&
        party.description.toLowerCase().includes(currentSearch.toLowerCase()));
    ...
    switch (currentFilter) {
      case "today":
        return (
          partyDate >= now && partyDate < new Date(today.getTime() + 86400000)
        );
      case "week":
    ...
    if (filteredParties.length === 0) {
      container.appendChild(
        EmptyState.create("parties", {
          ...
          message: "There are no parties for the selected filters",
  ...
  searchInput.addEventListener(
    ...
    debounce((e) => {
      ...
      if (currentSearch && filteredParties.length > 0) {
        ToastManager.info(
          `Found ${filteredParties.length} party${filteredParties.length !== 1 ? "s" : ""} matching "${currentSearch}"`,
        );
      }
    }, 300),
  );
...
function initFilters() {
  ...
  filterChips.addEventListener("click", (e) => {
    ...
    ToastManager.info(
      filterMessages[currentFilter] || `Filter: ${currentFilter}`,
    );
