...
function populateForm(user) {
  ...
  // Load profile picture - always use API endpoint (returns default if none exists)
...
async function submitForm() {
  ...
  if (!userId) {
    showError("No user logged in. Please log in first.");
    return;
  }
  ...
  try {
    // Only update user data - profile picture is uploaded immediately on selection
    const formData = {
...
async function uploadProfilePicture(userId) {
  console.log("uploadProfilePicture called, userId:", userId, "file:", selectedProfilePictureFile);
...
function handleFileSelect(event) {
  ...
  if (!userId) {
    showError("Please log in to upload profile picture");
    return;
  }
  ...
  // Preview the image immediately (local preview)
  ...
  // Upload immediately - don't wait for form submit
  ...
  if (userId) {
    ...
    console.warn("No userId, cannot upload");
  }
}
