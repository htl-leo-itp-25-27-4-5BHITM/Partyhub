# Invitation and attendance evidence and proposed contract

Group 5 review, 2026-09-25, against the unchanged application-source snapshot `9487ccb90bb438e24b3cfab547a5dc900b11aecb` from repository checkpoint `4e919f24779b71f8ab23bca6378565965b8ee143`. This document separates accepted D001/D004-D007/D012/D016 behavior, source observations, and the planning-only `document-invitations-and-attendance` contract. No application, API, browser, iOS, database, or runtime test was executed.
...
The proposed contract uses one logical invitation per `(party, recipient)` with `PENDING`, `ACCEPTED` and `DECLINED` states. Storage may retain history, but a declined or withdrawn invitation is not a current private-visibility grant.

| Action / prior state | Authorized actor and eligibility | Proposed next state | Membership / visibility effect | Event input | Observed source |
|---|---|---|---|---|---|
...
This disposition resolves the invitation portion of Q003 in the proposed child without defining host moderation or banning retained invitation history. Public-party invitation eligibility beyond stored-host authority is not expanded by inference; the existing private-invitation contract remains the normative scope.

## Invitation and attendance transition table

| Action / prior state | Actor and party eligibility | Proposed invitation state | Proposed membership | Visibility result | Event input | Repeat behavior |
|---|---|---|---|---|---|---|
...
| Projection | Proposed audience | Minimum content / rule | Current observation |
|---|---|---|---|
...
These proposed audiences resolve Q010 for invitations/attendance. They do not decide `can-edit` metadata beyond PARTY-05 host authority, profile field projections beyond SOC-05, or attendee location exposure owned by Step 10.
...
The evidence requires a bounded modification of `party-discovery-and-management`: expand the existing invitation and attendance requirements, define actor-scoped projections, and define transition event inputs. The child [proposal](../../openspec/changes/document-invitations-and-attendance/proposal.md), [design](../../openspec/changes/document-invitations-and-attendance/design.md), [delta](../../openspec/changes/document-invitations-and-attendance/specs/party-discovery-and-management/spec.md) and [tasks](../../openspec/changes/document-invitations-and-attendance/tasks.md) are planning-complete with **0/7 apply tasks complete**. Strict child validation passes.

The accepted main AUTH/SOC/PARTY requirements remain unchanged at **44 requirements/172 scenarios**, including `party-discovery-and-management` at **13/60**. Exact later sync would modify PARTY-06/PARTY-07, add PARTY-14/PARTY-15, and project the party capability to **15/88** and the full baseline to **46/200**. Q003 and Q010 have proposed answers but are not resolved until apply. Q004 and Q014 remain unresolved at their stated boundaries; G006/G009/G017 and G031-G033 retain implementation/client discrepancies.
